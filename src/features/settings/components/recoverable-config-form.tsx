...
interface RecoverableConfigFormProps {
  form: any;
}

export const RecoverableConfigForm = ({ form }: RecoverableConfigFormProps) => {
  const { control } = form;

  return (
    <TooltipProvider>
      <section className="flex flex-col gap-4 p-6 border rounded-lg w-full">
        <div className="flex items-center">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button type="button" className="ml-2 p-1 text-blue/80">
                <InfoCircle />
              </button>
            </TooltipTrigger>
            <TooltipContent
