...
import { useWarehousesForCreate } from "@/features/create";
...
export const StepDataWipeDetails: React.FC<StepDataWipeDetailsProps> = ({
  ...
  const { warehouses } = useWarehousesForCreate();
  ...
  const destinationOptions = React.useMemo(() => {
    ...
    const safeWarehouses = (Array.isArray(warehouses) && warehouses.length > 0) ? warehouses : [];
    ...
    const warehouseOptions = safeWarehouses.map((warehouse) => ({
      display: `${warehouse.name} (${warehouse.country})`,
    ...
    return groups;
  }, [members, sortedOffices, warehouses]);
  ...
  const getDestinationDisplayValue = (detail: DataWipeDetail) => {
    ...
    if (destinationType === "FP warehouse" && warehouse) {
      return `warehouse_${warehouse.warehouseId}`;
    }
    return "";
  };

  const handleDestinationChange = (assetId: string, selectedValue: string) => {
    if (!selectedValue) {
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 space-y-4">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <SelectDropdownOptions
                      ...
                      onChange={(value) => handleDestinationChange(asset._id, value)}
                      searchable={true}
                    />
                  </div>
                </div>
