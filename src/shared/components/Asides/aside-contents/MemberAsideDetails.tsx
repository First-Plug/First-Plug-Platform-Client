...
import { RelacoteProducts, ReturnPage } from "./";
import { GenericAlertDialog } from "@/features/assets";
import { DeleteMemberModal } from "@/shared";
import { useFetchMember, useMemberStore } from "@/features/members";
...
export const MemberAsideDetails = ({ className }: MemberAsideDetailsProps) => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useFetchMember(selectedMember?._id);
  ...
  const handleRequestOffBoarding = () => {
    ...
    if (!getMissingFields(member).length) {
      ...
      router.push(`/home/my-team/request-off-boarding/${member._id}`);
      setAside(undefined);
    } else {
      ...
      {relocatePage ? (
        ...
        <Fragment>
          <div className="flex flex-col gap-6 h-full">
            <MemberDetail memberId={selectedMember?._id} />
          ...
          <GenericAlertDialog
            ...
            onButtonClick={() => {
              queryClient.setQueryData(["selectedMember"], member);
              setAside("EditMember");
